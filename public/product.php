...
if (isset($_GET['remove'])) {
    ...
    $params = $_GET;
    // mshen m a3ml add or remove mrten hone
    unset($params['remove'], $params['add'], $params['addToCart']);
    // hone la3ml shkla shkl url query
...
if (isset($_GET['add']) && $_GET['add'] == 1) {
    ...
    // Validate quantity
    if ($qty < 1) {
        $_SESSION['stock_error'] = "Quantity must be at least 1!";
        $params = $_GET;
    ...
    $stock_check = $connect->prepare("SELECT stock FROM products WHERE id = ?");
    ...
    if ($stock_result->num_rows > 0) {
        ...
        if ($available_stock > 0) {
            ...
            // hone ya3ne ade ana m5tar llcart 
            $cart_total_qty = 0;
            ...
            // Calculate how many more we can add weza fye zeda aw l a
            $can_add = $available_stock - $cart_total_qty;
            
            if ($can_add <= 0) {
                $_SESSION['stock_error'] = "You already have all available stock in your cart!";
            } elseif ($qty > $can_add) {
                $_SESSION['stock_error'] = "You can only add $can_add more items (you already have $cart_total_qty in cart)!";
            } else {
                $found = false;
                // Check if item with same id, color, and size exists
                // & hateta bs lelak ene 3amel mtl pointer references
                foreach ($_SESSION["cart"] as &$v) {
                    if ($v['id'] == $id && $v['color'] == $color && $v['size'] == $size) {
                        // eza 3ende nfs item bs zid qty
            ...
            $_SESSION['stock_error'] = "This product is currently out of stock!";
        }
    } else {
...
$category_name = '';
$product_type = '';
$cart_total_qty = 0; // Initialize cart total quantity

if ($product_id) {
    $stmt = $connect->prepare("
            SELECT p.*, c.name as category_name, i.filename, 
            (SELECT name FROM categories WHERE id = p.category_id) as main_category
            FROM products p 
    ...
    if ($result->num_rows > 0) {
        ...
        $stock_available = $product_details['stock'] ?? 0;
        $category_name = $product_details['category_name'] ?? '';
        $product_type = $product_details['type'] ?? '';
        
        // wshyek ade fi bl cart abel m nshtre mshen m ysir fi 8lt lb qty
        foreach ($_SESSION["cart"] as $item) {
                        ...
                        foreach ($_SESSION['cart'] as $item):
                            ...
                            <li class="list-group-item border-0">
                                <div class="row align-items-center g-3">
                                    <div class="col-3">
                                        <img src="assets/images/<?= htmlspecialchars($item['photo']) ?>"
                                    ...
                                    <div class="col-9">
                                        ...
                                        <?php if (!empty($item['color'])): ?>
                                            <span class="badge bg-light text-dark">Color: <?= htmlspecialchars($item['color']) ?></span>
                                        <?php endif; ?>
                                        <?php if (!empty($item['size'])): ?>
                                            <span class="badge bg-light text-dark">Size: <?= htmlspecialchars($item['size']) ?></span>
                                        <?php endif; ?>
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="product-images">
                        <div class="main-product-img" id="mainProductContainer">
                            <img id="mainProductImage" src="assets/images/<?php if (isset($_GET["photo"])) {
                                echo $_GET["photo"];
                            } else {
                                echo "";
                            }
                            ?>" alt="<?php if (isset($_GET["name"])) {
                                echo $_GET["name"];
                ...
                <div class="image-zoom-modal" id="imageZoomModal">
                    ...
                    <div class="zoom-modal-content">
                        ...
                        <div class="zoom-controls">
                            ...
                            <input type="range" min="100" max="500" value="100" class="zoom-slider" id="zoomSlider"
                ...
                <div class="col-lg-6">
                    <div class="product-info">
                        <h1><?= htmlspecialchars($product_details['name'] ?? 'Product Not Found') ?></h1>
                        <div class="product-price">$<?= number_format($product_details['price'] ?? 0, 2) ?></div>

                        <!-- Stock Status -->
                        <!-- lnshf ade 3ena bl stock ba3ed fi b stock -->
                        <div class="stock-status">
                            <?php if ($stock_available > 0): ?>
                                <span class="in-stock">
                                    <i class="fas fa-check-circle"></i>
                                    In Stock (<?= $stock_available ?> available)
                                </span>
                                <?php if ($cart_total_qty > 0): ?>
                                    <div class="cart-quantity-info">
                                        <i class="fas fa-shopping-cart"></i> 
                                        You have <?= $cart_total_qty ?> of these in your cart
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                                ...
                                <p class="text-muted mt-2">This product is currently unavailable for purchase.</p>
                            <?php endif; ?>
                        </div>

                        <!-- Display stock error eza fi shi b stock aw 5lso -->
                        <?php if (isset($_SESSION['stock_error'])): ?>
                            <div class="stock-error show" id="stockError">
                                <?= htmlspecialchars($_SESSION['stock_error']) ?>
                            </div>
                        ...
                        <?php if (isset($_SESSION['notification_success'])): ?>
                            <div class="notification-success">
                                <?= htmlspecialchars($_SESSION['notification_success']) ?>
                                <br>
                        ...
                        <div class="form-grid">
                            ...
                            <form method="GET" class="display-content-form">
                                <div class="form-row">
                                    ...
                                    <select name="size" id="size-select" class="custom-input" required>
                                ...
                                <div class="form-row">
                                    ...
                                    <select name="color" id="color-select" class="custom-input" required>
                                        <option value="">Choose an option</option>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="quantity-selector small-qty">
                                        <button type="button" class="qty-btn" onclick="decreaseQuantity()" 
                                                id="decreaseBtn" <?= $stock_available == 0 ? 'disabled' : '' ?>>
                                            <i class="fa fa-minus"></i>
                                        </button>
                                        <!-- hone hn5le yfwet mn 1 l stock_av - ade fi cart bs ysfer ba3mla desable  -->
                                        <input class="qty-input" name="qty" type="number" value="1" id="quantity"
                                            min="1" max="<?= max(0, $stock_available - $cart_total_qty) ?>"
                                            <?= $stock_available == 0 ? 'disabled' : '' ?>>
                                        <button type="button" class="qty-btn" onclick="increaseQuantity()"
                                                id="increaseBtn" <?= $stock_available == 0 ? 'disabled' : '' ?>>
                                            <i class="fa fa-plus"></i>
                                        </button>
                                    </div>
                                    <!-- hone lyshf max ade ba3ed fi yzed  -->
                                    <?php if ($stock_available > 0 && ($stock_available - $cart_total_qty) > 0): ?>
                                        <div class="max-quantity-info">
                                            Max: <?= $stock_available - $cart_total_qty ?> available to add
                                        </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="form-row">
                                    <label></label>
                                    <button type="submit" name="addToCart" class="add-to-cart-btn small-btn"
                                        id="addToCartBtn" 
                                        <?= $stock_available == 0 ? 'disabled style="background: #cccccc !important;"' : '' ?>>
                                        <i class="fa fa-shopping-cart me-2"></i> 
                                        <?= $stock_available == 0 ? 'Out of Stock' : 'Add to Cart' ?>
                                    </button>
                                </div>
                            </form>
                        </div>

                        <!-- Notify When Available Form (Only show if out of stock) -->
...
$result=$stmt->get_result();
    ...
    <script>
        ...
        function increaseQuantity() {
            ...
            const stockAvailable = <?= $stock_available ?>;
            const cartTotalQty = <?= $cart_total_qty ?>;
            
            if (stockAvailable === 0) return;
            
            const maxQuantity = stockAvailable - cartTotalQty;
            ...
            if (currentValue < maxQuantity) {
                ...
                if (currentValue + 1 >= maxQuantity) {
                    ...
                    showStockError(`You can only add ${maxQuantity} items maximum!`);
                }
            } else {
                showStockError(`You can only add ${maxQuantity} items maximum!`);
            }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initialize quantity input with proper validation
            const quantityInput = document.getElementById('quantity');
            const stockAvailable = <?= $stock_available ?>;
            const cartTotalQty = <?= $cart_total_qty ?>;
            ...
            const addToCartForm = document.querySelector('form.display-content-form');
            if (addToCartForm) {
                addToCartForm.addEventListener('submit', function(e) {
                    ...
                    if (!sizeSelect.value || !colorSelect.value) {
                        e.preventDefault();
                        showStockError('Please select both size and color!');
                        return false;
                    }
                    ...
                    if (qty > maxQuantity) {
                        e.preventDefault();
                        showStockError(`You can only add ${maxQuantity} more items!`);
                        return false;
                    }
            ...
            // Update notify form with selected values
            ...
            if (notifyForm) {
                ...
                sizeSelect.addEventListener('change', function() {
                    notifyForm.querySelector('input[name="size"]').value = this.value;
                });
                
                colorSelect.addEventListener('change', function() {
                    notifyForm.querySelector('input[name="color"]').value = this.value;
                });
            }
        });
        ...
        function showStockError(message) {
            ...
            if (!errorDiv) {
                ...
                document.querySelector('.product-info').insertBefore(errorDiv, document.querySelector('.form-grid'));
            }
